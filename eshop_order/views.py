...
def order(request):
    order_form=UserNewOrderForm(request.POST or None)
    if order_form.is_valid():
        order=Order.objects.filter(owner_id=request.user.id,is_paid=False).first()
        if order is None:
            order=Order.objects.create(owner_id=request.user.id,is_paid=False)

        product_idd=order_form.cleaned_data.get("productId")
        count=order_form.cleaned_data.get('count')
        if count<0:
            count=1
        product=Product.objects.get_by_id(product_id=product_idd)

        order.orderdetail_set.create(product_id=product.id,count=count,price=product.price)
        product.bestselling+=1
        product.save()
        return redirect(f'/product/{product_idd}')
...
def user_open_order(request):
    ...
    return render(request,'open_order/cart.html',context)
